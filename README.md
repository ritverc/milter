...
т.п.) префиксы темы могли бы накапливаться: `[FAKE-REPLY] [FAKE-REPLY] Hello`.

Чтобы этого избежать, при срабатывании правил milter сначала **вырезает** из
начала темы все уже присутствующие «известные» префиксы (любой
`subject_prefix`, объявленный любым триггером или группой в конфигурации —
включён или нет), и лишь затем добавляет новый комбинированный префикс. Таким
образом тема всегда соответствует текущей оценке правил и не дублирует
префиксы.
...
Пример:
- Тема до обработки: `[FAKE-REPLY] Re: Project update`
- Срабатывает триггер `false_reply` → префикс `[FAKE-REPLY]`
- Без вырезания: `[FAKE-REPLY] [FAKE-REPLY] Re: Project update`
- С вырезанием: `[FAKE-REPLY] Re: Project update`
